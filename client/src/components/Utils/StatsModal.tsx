...
const StatsModal = (props: any) => {
  ...
  const { userGenres, userArtists, userFriends, setGraphUserId } =
  ...
  return (
    <>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        <ModalOverlay />
        <ModalContent bg={bg} maxW='600'>
          <ModalHeader>Charts & Stuff</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <SimpleGrid p={15} columns={1} borderRadius='15px'>
              {userGenres && (
                <PieChart
                  cusTheme={props.cusTheme}
                  graphData={userGenres}
                  graphName={'genres'}
                  userName={userName}
                  key={1}
                />
